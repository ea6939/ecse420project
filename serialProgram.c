...
int main(int argc, char *argv[]) {
	...
	step_size_initial = atoi(argv[2]);
	
	int i,j;
	
	// get width and height from file
	FILE *image;
	image=fopen("./f1.txt","rb+");
	if(image==NULL)
	{
	...
	//printf("prev img:\n");
	import_image_from_text(img_prev, "./f1.txt");
	
	//printf("curr img:\n");
	import_image_from_text(img_curr, "./f2.txt");
...
//Input curr: 0 to calculate first image, 1 for second image
RGB compute_sum_rgb_avg(int k, int l, int curr) {
	int i, j;
	int total_R = 0;
	int total_G = 0;
	int total_B = 0;
	...
	a.R = total_R / (block_size*block_size);
	a.G = total_G / (block_size*block_size);
	a.B = total_B / (block_size*block_size);
	//printf("Block %d %d %d: %d\n", k, l, curr, total_RGB);
	return a;
}
...
void process_images_using_avg() {
	...
	for(k = 0; k < m / block_size; k++) { // rows
		for(l = 0; l < n / block_size; l++) { // columns
			...
			// if out of bounds
			a = compute_sum_rgb_avg(curr_k, curr_l, 0);
			if(a.R == -1) {
				printf("OoB");
				continue;
			}
			...
			//If the block is the same (no movement detected)
			a = compute_sum_rgb_avg(curr_k, curr_l, 0);
			b = compute_sum_rgb_avg(curr_k, curr_l, 1);
			if(compute_sum_rgb(curr_k, curr_l, 0) - compute_sum_rgb(curr_k, curr_l, 1) == 0) {
				motion_vectors[k][l].x = 0;
				motion_vectors[k][l].y = 0;
				//printf("(0,0)");
